  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6">
          {activeTab === 'summary' && <ExecutiveSummaryTab statistics={statistics} insights={insights} />}
          {activeTab === 'detailed' && <DetailedAnalysisTab products={products} />}
          {activeTab === 'risk' && <RiskAssessmentTab statistics={statistics} products={products} />}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {categories.map((category, index) => {
              const percentage = ((category.total_quantity / statistics.totalQuantity) * 100).toFixed(1);
              ...
              return (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm">{category.product_count}</td>
                  <td className="px-6 py-4 text-sm">{category.total_quantity.toLocaleString()}</td>
                  <td className="px-6 py-4 text-sm">{percentage}%</td>
                  <td className="px-6 py-4 text-sm">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Products</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Quantity</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">EOL Products</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">AI Enhanced</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {manufacturers.map((mfr, index) => (
              <tr key={index} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm font-medium">{mfr.manufacturer || 'Unknown'}</td>
                <td className="px-6 py-4 text-sm">{mfr.product_count}</td>
                <td className="px-6 py-4 text-sm">{mfr.total_quantity.toLocaleString()}</td>
                <td className="px-6 py-4 text-sm">{mfr.eol_count || 0}</td>
                <td className="px-6 py-4 text-sm">{mfr.ai_enhanced_count || 0}</td>
                <td className="px-6 py-4 text-sm">{mfr.avg_confidence || 0}%</td>
              </tr>
            ))}
          </tbody>
        </table>
