...
const Phase3Results = ({ phase2JobId, isActive, customerName, onComplete, onResearchComplete }) => {
  ...
  // Add these after the existing useRef declarations
const [statusCheckInterval, setStatusCheckInterval] = useState(null);
const [hasDetectedCompletion, setHasDetectedCompletion] = useState(false);
  ...
  useEffect(() => {
    if (isActive && phase2JobId && !phase3JobId) {
      initializePhase3();
    } else if (isActive && phase3JobId) {
      // When becoming active with an existing job, check if it's already complete
      checkJobStatus();
    }
  ...
  // Add this new function to check job completion status
const checkJobStatus = async () => {
  if (!phase3JobId || hasDetectedCompletion) {
    return;
  }
  
  try {
    console.log('Checking job status for completion...');
    const response = await fetch(`${API_BASE_URL}/api/phase3/results/${phase3JobId}`);
    const data = await response.json();
    ...
    if (response.ok && data.job) {
      ...
      // Check if job is completed based on status or statistics
      const isCompleted = 
        job.status === 'completed' || 
        (stats.totalProducts > 0 && stats.aiEnhanced === stats.totalProducts) ||
        (job.processed_products > 0 && job.processed_products === job.total_products);
      
      if (isCompleted && !hasDetectedCompletion) {
        console.log('✅ Job completion detected via status check!');
        console.log('Job status:', job.status);
        console.log('Stats:', stats);
        
        // Mark as completed
        setHasDetectedCompletion(true);
...
const startStatusPolling = () => {
  ...
  console.log('Starting status polling for completion detection...');
...
const stopStatusPolling = () => {
  ...
  const runAIResearch = async () => {
    ...
    setHasDetectedCompletion(false); // Reset completion flag
    
    // Start status polling as backup completion detection
    startStatusPolling();
  ...
  const setupEnhancedSSEConnection = () => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        // Manually trigger completion since completed flag might not be set
        setTimeout(() => {
          console.log('Forcing completion based on message content');
          setResearchStatus('completed');
          setResearchingProductId(null);
          
          // Call the completion callback
          if (onResearchComplete) {
            console.log('Calling onResearchComplete callback (forced)');
            onResearchComplete();
          }
        ...
        // In the progress message handler, update the completion check:
        if (progress.completed || (progress.processed === progress.total && progress.total > 0)) {
          console.log('Research complete detected via SSE!');
          eventSource.close();
          setResearchStatus('completed');
          setResearchingProductId(null);
          setHasDetectedCompletion(true); // Mark as detected
          
          // Stop status polling since we detected completion
          stopStatusPolling();
          
          // Call the completion callback
          if (onResearchComplete) {
            console.log('Calling onResearchComplete callback from SSE');
            onResearchComplete();
          }
    ...
    eventSource.onerror = (error) => {
      ...
      // When SSE errors/closes, check completion status
      console.log('SSE connection lost, checking completion status...');
      
      // Do an immediate status check
      setTimeout(async () => {
        const completed = await checkJobStatus();
      
      if (!completed && !hasDetectedCompletion) {
        // If not completed, check a few more times
        setTimeout(() => checkJobStatus(), 2000);
        setTimeout(() => checkJobStatus(), 5000);
        setTimeout(() => {
          checkJobStatus();
          // Stop polling after final attempts
          stopStatusPolling();
        }, 8000);
      }
    }, 1000);
    ...
    setResearchProgress(prevProgress => {
      if (prevProgress.processed > 0) {
        ...
        // Don't call onResearchComplete here - let checkJobStatus handle it
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3 mb-4">
          <div 
            className="bg-gradient-to-r from-purple-600 to-indigo-600 h-3 rounded-full transition-all duration-500 progress-bar-fill"
            style={{ width: `${researchProgress.percentComplete || 0}%` }}
          />
        </div>
    ...
    return (
      <React.Fragment key={product.product_id}>
        ...
        >
          ...
          <td className="px-4 py-4 text-center">
            {product.ai_enhanced && (
              <Brain className="h-4 w-4 text-purple-600 mx-auto ai-enhanced-glow" />
            )}
          </td>
        </tr>
      ...
      <div className="bg-gradient-to-r from-purple-700 to-indigo-800 text-white p-6 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-sm text-purple-200">Customer</div>
            ...
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium mt-2 ${
              researchStatus === 'researching' ? 'bg-yellow-400 text-yellow-900' :
              researchStatus === 'completed' ? 'bg-green-400 text-green-900' :
