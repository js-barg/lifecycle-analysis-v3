...
const phase3Controller = {
  async initializePhase3(req, res) {
    ...
    try {
      ...
      itemsToProcess.forEach(item => {  // ← Using filtered items here
    ...
    try {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        try {
          ...
          console.error(`❌ Failed to research ${product.product_id}:`, error.message);
          failureCount++;
          processedCount++;
  ...
  try {
    ...
    if (cached && !cached.isExpired) {
      console.log(`📦 Cache hit: ${product.product_id}`);
      return {
        ...product,
  ...
  async runAIResearchInternal(products, jobId) {
    console.log(`🚀 Starting AI research for ${products.length} products in job ${jobId}`);
...
if (processedResult.end_of_sale_date || processedResult.last_day_of_support_date) {
  ...
  if (processedResult.last_day_of_support_date && !processedResult.end_of_sale_date) {
    ...
    console.log(`📅 Calculated EOS from LDOS: ${processedResult.end_of_sale_date}`);
  }
  ...
  console.log(`📊 Dates after calculation: EOS=${processedResult.end_of_sale_date}, LDOS=${processedResult.last_day_of_support_date}`);
}
  ...
  async storePhase3Result(result) {
    ...
    try {
      ...
      console.log(`✅ Stored Phase 3 result for ${result.product_id}`);
    } catch (error) {
      console.error(`❌ Failed to store Phase 3 result for ${result.product_id}:`, error.message);
      throw error;
    }
  },
